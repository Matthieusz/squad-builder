<script lang="ts">
	...
	const minLevel = $derived(
		squadCharacters.length > 0 ? Math.min(...squadCharacters.map(({ character }) => character.lvl)) : 0
	);

	const maxLevel = $derived(
		squadCharacters.length > 0 ? Math.max(...squadCharacters.map(({ character }) => character.lvl)) : 0
	);
...
<svelte:head>
	<title>
		{squad ? `${squad.name} - ${group?.name ?? ''} - Squad Builder` : 'Squad Not Found'}
	</title>
</svelte:head>
...
{#if !group || !squad}
	<div class="flex h-full items-center justify-center p-4">
		<div class="rounded-lg border border-dashed p-8 text-center">
			<h1 class="mb-2 text-xl font-semibold">Squad Not Found</h1>
			...
			<div class="flex items-center gap-2 border-b p-3">
				...
				<div class="min-w-0 flex-1">
					...
					<p class="text-xs text-muted-foreground">
						{squad.characterIds.length}/10 characters
					</p>
				</div>
			</div>
				...
				<div class="mb-3 flex items-start justify-between">
					<div>
						...
						<p class="text-sm text-muted-foreground">
							{group.name} • {squad.characterIds.length} character{squad.characterIds.length === 1 ? '' : 's'}
						</p>
					</div>
				...
				{#if squadCharacters.length > 0}
					<div class="flex flex-wrap items-center gap-3 text-sm">
						<div class="flex items-center gap-1">
							...
							<span class="font-medium">{minLevel} - {maxLevel}</span>
						</div>
						<div class="flex items-center gap-1">
							<span class="text-muted-foreground">Professions:</span>
				...
				{#if squadCharacters.length === 0}
					<div class="flex h-full items-center justify-center text-muted-foreground">
						<div class="text-center">
							<p class="mb-2">No characters in this squad yet</p>
							<p class="text-sm">Select characters from the left panel to add them</p>
						</div>
					</div>
				{:else}
					<!-- 2x5 Grid -->
