<script lang="ts">
	...
	import SquadEditor from '$lib/components/SquadEditor.svelte';
	import { IconArrowLeft, IconPlus, IconUsers } from '@tabler/icons-svelte';
...
{:else}
	<div class="container mx-auto max-w-7xl p-4">
		...
		{#if group.squads.length === 0}
			<div class="rounded-lg border border-dashed p-8 text-center">
				<p class="text-sm text-muted-foreground">No squads in this group yet.</p>
				<p class="text-xs text-muted-foreground/70">
					Click "Add Squad" to create your first squad.
				</p>
			</div>
		{:else}
			<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{#each group.squads as squad (squad.id)}
					<SquadEditor {group} {squad} />
				{/each}
			</div>
		{/if}
	</div>
