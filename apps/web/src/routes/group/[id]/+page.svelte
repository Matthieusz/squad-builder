<script lang="ts">
    ...
    import { IconArrowLeft, IconPlus } from "@tabler/icons-svelte";
    ...
    const totalCharacters = $derived(
        group?.squads.reduce(
            (sum, squad) => sum + squad.characterIds.length,
            0,
        ) ?? 0,
    );
...
<svelte:head>
    <title
        >{group
            ? `${group.name} - Kreator Drużyn`
            : "Grupa nie znaleziono"}</title
    >
</svelte:head>
...
{#if !group}
    <div class="flex h-full items-center justify-center p-4">
        <div class="rounded-lg border border-dashed p-8 text-center">
            <h1 class="mb-2 text-xl font-semibold">Grupy nie znaleziono</h1>
            <p class="mb-4 text-sm text-muted-foreground">
                Grupa, której szukasz, nie istnieje lub została usunięta.
            </p>
    ...
    <div class="container mx-auto max-w-5xl p-4">
        ...
        {#if group.squads.length === 0}
            <div class="rounded-lg border border-dashed p-12 text-center">
                <p class="text-muted-foreground">Brak squadów w tej grupie.</p>
                <p class="mt-1 text-sm text-muted-foreground/70">
                    Kliknij "Nowy squad" aby utworzyć pierwszy squad.
                </p>
            </div>
        {:else}
...
<!-- Create Squad Dialog -->
<Dialog
    open={showCreateSquad}
    onOpenChange={(open) => (showCreateSquad = open)}
>
    <DialogContent>
        <DialogHeader>
            <DialogTitle>Nowy squad</DialogTitle>
        </DialogHeader>

        <div class="space-y-4 py-4">
            <div class="space-y-2">
                <label for="squad-name" class="text-sm font-medium"
                    >Nazwa squadu</label
                >
                <Input
                    id="squad-name"
                    bind:value={newSquadName}
                    placeholder="np. Główny squad"
                />
