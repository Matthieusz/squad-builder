<script lang="ts">
    ...
    function getAccountName(id: string): string {
        return (
            appStore.accounts.find((a) => a.id === id)?.name ?? "Nieznane konto"
        );
    }
</script>
...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h2 class="text-lg font-semibold">Konta</h2>
    ...
    {#if appStore.accounts.length === 0}
        <div class="rounded-lg border border-dashed p-8 text-center">
            <p class="text-sm text-muted-foreground">
                Nie zaimportowano jeszcze żadnych kont.
            </p>
            <p class="text-xs text-muted-foreground/70">
                Kliknij „Dodaj konto”, aby zaimportować postacie ze strony
                profilu.
            </p>
        </div>
    {:else}
...
<Dialog
    ...
    onOpenChange={() => (accountToDelete = null)}
>
    <DialogContent>
        <DialogHeader>
            <DialogTitle>Usuń konto</DialogTitle>
            <DialogDescription>
                Czy na pewno chcesz usunąć "{accountToDelete
                    ? getAccountName(accountToDelete)
                    : ""}"? To usunie wszystkie
                {accountToDelete
                    ? (appStore.accounts.find((a) => a.id === accountToDelete)
                          ?.characters.length ?? 0)
                    : 0}
                postaci i usunie je ze wszystkich składów. Tej operacji nie można
                cofnąć.
            </DialogDescription>
        </DialogHeader>
        <DialogFooter>
